...
class TestStreakManager(unittest.TestCase):
    ...
    def test_daily_target_randomization_range(self):
        targets = set()
        for i in range(100):
            state = {}
        ...
        # Over 100 trials, all values 2,3,4,5 should be observed
    ...
    def test_forced_commit(self):
        ...
        fake_now = datetime(2026, 9, 2, 23, 0, tzinfo=timezone.utc)
    ...
    def test_daily_target_reached_skips(self):
        ...
        fake_now = datetime(2026, 9, 2, 10, 0, tzinfo=timezone.utc)
    ...
    def test_guaranteed_commit_when_slots_low(self):
        # If it's hour 22 (only 1 slot left) and we have 0 commits of a target 2
        state = {
            "current_date": "2026-09-02",
            "daily_target": 2,
            "commits_today": 0
        }
        fake_now = datetime(2026, 9, 2, 22, 0, tzinfo=timezone.utc)
        should_commit, reason = streak_manager.decide_commit(state, fake_now, force=False)
        self.assertTrue(should_commit)
    ...
    def test_date_rollover_resets_counter(self):
        ...
        fake_now = datetime(2026, 9, 2, 1, 0, tzinfo=timezone.utc)
